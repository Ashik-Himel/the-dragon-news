  ...
  return (
    <aside className="bg-white w-[300px] md:w-full fixed top-0 -left-full bottom-0 md:sticky md:top-[96px] overflow-y-auto z-30 md:z-10 px-4 pb-8 pt-14 md:p-0 border-e-2 md:border-0 border-gray [box-shadow:15px_0px_30px_0px_rgba(0,0,0,0.4)] md:shadow-none transition-[left]" style={leftSidebarShow ? {left: '0'} : {}}>
      ...
      <section>
        ...
        {
          categories.map(category => <NavLink to={`/${category.id}`} key={category.id} className="block p-4 text-center text-light-gray" style={({isActive}) => isActive || (pathname === '/' && category.id === '0') ? {backgroundColor: '#E7E7E7'} : null} onClick={() => setLeftSidebarShow(!leftSidebarShow)}>{category.name}</NavLink>)
        }
      </section>
      {/* News Card Section */}
